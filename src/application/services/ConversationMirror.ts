...
/**
 * ConversationMirror —— 線性 chat ↔ CLI 雙向 relay。
 * 純 application 邏輯，只依賴 port，無 I/O，不認識底層是哪支 CLI。
 */
export class ConversationMirror {
  // 記最近一則「chat → CLI」的問題；下一次 CLI emit exchange 的 user 命中時只回 response，
  ...
  async start(): Promise<void> {
    this.bot.onMessage((text) => this.forwardToCli(text))
    this.cliIO.onMessage((ex) => this.forwardToBot(ex))
  ...
  private forwardToCli(text: string): void {
    this.log.info(`[mirror] chat→cli: ${text.length} chars`)
  ...
  private forwardToBot(ex: PaneExchange): void {
    ...
    this.log.info(`[mirror] cli→chat: ${payload.length} chars`)
    if (!payload) return
